...
| **ActorRef** | on every recorded act | role, version, mode (agent \| human \| program), principal (the authenticated identity behind the act — the named human whenever mode = human), office (when acting in Board capacity) | No anonymous acts: identity and capacity are recorded per act (§5, HB-3). |
...
| C11 | §5: the registry holds identity (with model snapshot), lineage, status (with probationary), and per-act ActorRef with principal/mode/office — human-held acts name the human | event-history sample |
